...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  // clear cart during user logout
  const HandleDeleteCartAndLogout = () => {
    if (CartCounter > 0) {
      axiosUser
        .delete(`user/cart`)
        .then((res) => {
      ...
      <div className="max-w-[1920px] w-full px-[264px] max-2xl:px-[160px] max-lg:px-[90px] max-sm:px-[25px] pb-[100px] flex flex-col gap-y-[100px] relative">
        <div className="flex flex-col gap-y-[30px] max-lg:gap-y-[25px]">
          ...
          <div className="flex gap-[25px] max-sm:gap-[10px] ">
            <div
              className={`rounded-[12px] bg-white flex flex-col justify-between py-[60px] max-lg:py-[40px] duration-200 max-md:hidden w-[330px] max-lg:w-[233px] px-[30px] max-lg:px-[20px] ${
                user?.id ? "" : "opacity-[0.8]"
              }`}
            >
              <div
            ...
            <div
              className={`rounded-[12px] bg-white max-lg:bg-transparent w-[calc(100%-355px)] max-lg:w-[calc(100%-233px)] max-[992px]:w-[calc(100%-65px)] max-md:w-full p-[20px] max-sm:p-[10px] `}
            >
              {children}
            </div>
          </div>
