...
export default function DropDown1value({
  ...
  useEffect(() => {
    if (searchable) {
      if (name == "streetName") {
        ...
        if (drop) {
          axiosUser
            .get(`testing/getStreet?name=${searchValue}`)
            .then(({ data }) => {
          ...
          setFilteredData(
            data.filter((item: any) =>
              item.name
                ...
                : item.SalaroName
                ? item.SalaroName.toLowerCase().startsWith(
                    searchValue?.toLowerCase()
                  )
                : item.ProdTypeName
                ? item.ProdTypeName.toLowerCase().startsWith(
                    searchValue?.toLowerCase()
                  )
                : item.ProdTypeGroupName.toLowerCase().startsWith(
                    searchValue?.toLowerCase()
                  )
            )
          );
  ...
  }, [data, searchable, searchValue, name, drop]);
  ...
  return (
    <div className="flex flex-col gap-y-[5px] w-full">
      ...
      <div ref={targetRef} className="relative w-full text-custgray h-full">
        ...
        >
          {searchable && (
            <div className="h-[40px] shrink-0 shadow mx-[5px] px-[8px] rounded-[5px] bg-white sticky top-0 z-[1]">
              <input
                ref={inputRef}
                type="text"
                value={searchValue}
          ...
          <div
            className={`relative ${
              filterLoader && "opacity-[0.5] pointer-events-none"
            }`}
          >
            {filterLoader && (
              <div className="w-[40px] h-[40px] flex items-center justify-center absolute top-[50px] left-[50%] translate-x-[-50%] z-[1]">
                <DotsLoader />
              </div>
            )}
              ...
              <p
                ...
                className={`px-[10px] flex items-center w-full h-[40px] text-[13px] truncate  cursor-pointer duration-100 select-none ${
                  ...
                  (item.name == value ||
                    ...
                    item.SalaroName == value ||
                    item.ProdSaxeobaName == value ||
                    item.ProdTypeName == value ||
                    item.ProdTypeGroupName == value)
                    ? "bg-gray-400"
                    : "hover:bg-gray-300"
                } ${index % 2 === 0 ? "bg-[#F5F7FC]" : ""}`}
              >
