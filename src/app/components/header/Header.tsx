...
export default function Header() {
  ...
  const { setActiveAuthorization, setBurgerMenu, menuRoutes } = useContext(
  ...
  return (
    ...
    <div className={`max-w-[1920px] w-full items-center justify-between py-[12px] pl-[340px] pr-[264px] max-lg:px-[90px] max-tiny:px-[25px] mb-[10px] ${
        (pathname.split("/")[1] === "products" ||
          pathname.split("/")[1] === "catalog-for-set") ?
        "max-2xl:pl-[160px] max-2xl:pr-[90px]" : "max-2xl:pl-[220px] max-2xl:pr-[160px] max-1.5xl:pl-[200px]"
          ...
          <ul className="flex items-center gap-[5px] h-full max-lg:hidden">
            {menuRoutes.map((item: any, index: number) => (
              <li
                ...
                className={`h-full rounded-full text-[14px] cursor-pointer flex items-center px-[25px] ${item.name === "FAQ" && "max-1.5xl:hidden"} ${
                  item.link === "favorites"
