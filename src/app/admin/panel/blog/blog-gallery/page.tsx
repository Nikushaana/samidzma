"use client";

import React, { useContext, useEffect, useState } from "react";
import { BsXLg } from "react-icons/bs";
import { FaTrashCan } from "react-icons/fa6";
import DotsLoader from "@/app/components/loaders/DotsLoader";
import { axiosAdmin } from "../../../../../../dataFetchs/AxiosToken";
import useBlogGallery from "../../../../../../dataFetchs/blogGalleryGetFetch";
import Image from "next/image";
import ReactPaginate from "react-paginate";
import { ContextForSharingStates } from "../../../../../../dataFetchs/sharedStates";

export default function Page() {
  const { allBlogGalleryRender, setAllBlogGalleryRender } = useContext(
    ContextForSharingStates,
  );

  const [allBlogGalleryData, setAllBlogGalleryData] = useState<any>([]);
  const [allBlogGalleryLoader, setAllBlogGalleryLoader] =
    useState<boolean>(true);

  const [BlogGalleryDeletePopUp, setBlogGalleryDeletePopUp] =
    useState<string>("");
  const [BlogGalleryDeleteLoader, setBlogGalleryDeleteLoader] =
    useState<string>("");

  const HandleDeleteBlogGallery = (id: any) => {
    setBlogGalleryDeleteLoader(id);
    axiosAdmin
      .delete(`admin/gallery/${id}`)
      .then((res) => {
        setAllBlogGalleryRender(res);
      })
      .catch((err) => {})
      .finally(() => {
        setBlogGalleryDeleteLoader("");
      });
  };

  const [currentPage, setCurrentPage] = useState(0);
  const [prodwholenum, setProdwholenum] = useState<any>();
  const pageCount = Math.ceil(prodwholenum / 20);

  const handlePageClick = (event: any) => {
    setCurrentPage(event.selected);
  };

  useEffect(() => {
    setAllBlogGalleryLoader(true);
    axiosAdmin
      .get(`admin/gallery?page=${currentPage + 1}&per_page=20`)
      .then((res) => {
        setAllBlogGalleryData(res.data.data);
        setAllBlogGalleryLoader(false);

        setProdwholenum(res.data.total);
      })
      .catch((err) => {})
      .finally(() => {});
  }, [allBlogGalleryRender, currentPage]);

  return (
    <div className="flex flex-col gap-y-[10px] items-center relative">
      <h1 className="w-full">ბლოგის გალერეა</h1>
      {allBlogGalleryLoader && (
        <div className="w-[60px] h-[60px] flex items-center justify-center absolute top-[150px] left-[50%] translate-x-[-50%] z-[1]">
          <DotsLoader />
        </div>
      )}{" "}
      {allBlogGalleryData?.length > 0 ? (
        <div className="w-full grid grid-cols-4 max-sm:grid-cols-2 gap-4">
          {allBlogGalleryData.map((item: any, index: number) => (
            <div
              key={item.id}
              className={`border-[1px] relative rounded-[10px] duration-100 w-full aspect-video overflow-hidden ${
                BlogGalleryDeleteLoader == item.id && "opacity-[0.5]"
              }`}
            >
              <Image
                src={`${process.env.NEXT_PUBLIC_API_URL}/${item?.image}`}
                alt={""}
                sizes="500px"
                fill
                style={{
                  objectFit: "contain",
                }}
              />
              <div className="absolute top-[5px] right-[10px] bg-white rounded-lg">
                {BlogGalleryDeleteLoader == item.id ? (
                  <div className="w-[50px] h-[50px] flex items-center justify-center">
                    <DotsLoader />
                  </div>
                ) : (
                  <div className="flex items-center gap-[10px]">
                    <div className="relative">
                      <div
                        onClick={() => {
                          setBlogGalleryDeletePopUp(item.id);
                        }}
                        className="w-[40px] h-[40px] rounded-full flex items-center justify-center text-[red] text-[18px] hover:bg-red-200 hover:shadow-md duration-300 cursor-pointer"
                      >
                        <FaTrashCan />
                      </div>
                      {BlogGalleryDeletePopUp == item.id && (
                        <div className="absolute top-[-5px] right-[0px] flex items-center gap-[10px] p-[10px] h-[50px] bg-[#f4f6f9] shadow-md rounded-[8px]">
                          <div
                            onClick={() => {
                              HandleDeleteBlogGallery(item.id);
                            }}
                            className="w-[40px] h-[40px] rounded-full flex items-center justify-center text-[red] text-[18px] hover:bg-red-200 hover:shadow-md duration-300 cursor-pointer"
                          >
                            <FaTrashCan />
                          </div>
                          <div
                            onClick={() => {
                              setBlogGalleryDeletePopUp("");
                            }}
                            className="w-[40px] h-[40px] rounded-full flex items-center justify-center text-[18px] hover:shadow-md duration-300 cursor-pointer"
                          >
                            <BsXLg />
                          </div>
                        </div>
                      )}
                    </div>
                  </div>
                )}
              </div>
            </div>
          ))}
        </div>
      ) : (
        <p>
          {allBlogGalleryLoader
            ? "ბლოგის გალერეა იძებნება.."
            : "ბლოგის გალერეა არ არსებობს"}
        </p>
      )}
      {prodwholenum > 20 && (
        <div className="pt-[20px] flex justify-center">
          <ReactPaginate
            breakLabel="..."
            nextLabel="next >"
            onPageChange={handlePageClick}
            pageRangeDisplayed={5}
            pageCount={pageCount}
            previousLabel="< previous"
            renderOnZeroPageCount={null}
            breakLinkClassName={""}
            breakClassName={"w-8 h-8 flex items-center justify-center"}
            //main container
            containerClassName={`flex items-center gap-1`}
            //single page number
            pageLinkClassName={`w-[40px] h-[40px] text-md bg-gray-100 font-forh
           flex items-center justify-center rounded-full`}
            //previous page number
            previousLinkClassName={`hidden`}
            //next page number
            nextLinkClassName={`hidden`}
            //active page
            activeLinkClassName={"!important text-[#CACACA] font-forh"}
            forcePage={currentPage}
          />
        </div>
      )}
    </div>
  );
}
