...
export default function Page() {
  ...
  useEffect(() => {
    ...
    axiosAdmin
      .get(
        `admin/user?page=${currentPage + 1}&per_page=20&key=${
          searchUsers.userSearchKey
        }`
      )
      .then((res) => {
      ...
      .catch((err) => {})
      .finally(() => {});
  }, [allAdminUsersRender, currentPage, searchUsers.userSearchKey]);
  ...
  return (
    <div className="flex flex-col gap-y-[10px] items-center relative">
      ...
      <div className="w-[400px] max-sm:w-full">
        <Input1
          title="იუზერების ფილტრი"
          name="userSearchKey"
          type="text"
          setAllValues={setSearchUsers}
        />
      </div>
